...
def encrypt(key, data):
    """Encrypt the "data" input using the "key".
    ...
    data_pad = padder.update(data)
    data_pad += padder.finalize()

    cipher = Cipher(algorithms.AES(key), modes.CBC(iv))
    encryptor = cipher.encryptor()
    data_enc = encryptor.update(data_pad) + encryptor.finalize()
    data_enc = iv + data_enc
    return data_enc


def decrypt(key, data):
    """Decrypt the "data" input using the "key".
    ...
    data_plain = decryptor.update(data) + decryptor.finalize()
    unpadder = padding.PKCS7(128).unpadder()

    data_unpad = unpadder.update(data_plain)
    data_unpad += unpadder.finalize()
    return data_unpad
